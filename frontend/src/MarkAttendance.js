...
function MarkAttendance() {
  ...
  const checkHolidayStatus = async () => {
    ...
    try {
      const response = await axios.get(`http://127.0.0.1:8000/check-holiday/${instituteName}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://127.0.0.1:8000/mark-attendance', formData);
