...
function DressCodeManager({ adminData }) {
  ...
  const fetchDressCodes = async () => {
    try {
      const response = await axios.get(`http://127.0.0.1:8000/admin/dress-codes/${adminData.institute_id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://127.0.0.1:8000/admin/dress-code/upload', formData);
  ...
  const handleDelete = async (dressCodeId) => {
    ...
    try {
      const response = await axios.delete(`http://127.0.0.1:8000/admin/dress-code/${dressCodeId}`);
