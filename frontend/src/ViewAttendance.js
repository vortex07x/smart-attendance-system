...
function ViewAttendance({ adminData }) {
  ...
  const fetchAttendance = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`http://127.0.0.1:8000/admin/attendance/${adminData.institute_id}/today`);
  ...
  const handleClearAttendance = async () => {
    ...
    try {
      const response = await axios.delete(`http://127.0.0.1:8000/admin/attendance/${adminData.institute_id}/clear`);
  ...
  const handleDownloadExcel = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `http://127.0.0.1:8000/admin/export-attendance/${adminData.institute_id}`,
        {
          responseType: 'blob',
        }
      );
